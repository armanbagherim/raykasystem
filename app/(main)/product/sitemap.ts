...
async function getProducts(page) {
  ...
  const url = `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/ecommerce/products?limit=${limit}&offset=${offset}`;
...
export default async function sitemap({ id }) {
  ...
  return products.result.map((product) => ({
    url: `${process.env.WEBSITE_BASE_URL}/product/${product.sku}/${product.slug}`,
