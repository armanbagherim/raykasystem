...
async function getEntity(categorySlug) {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/eav/admin/entityTypes/slug/${categorySlug}`,
        {
            cache: "no-store",
        }
    );
...
async function getBrand(brandSlug) {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/ecommerce/brands/slug/${brandSlug}`,
        {
            cache: "no-store",
        }
    );
...
async function getProducts(entity, brand, searchParams) {
    const queryString = Object.entries(searchParams)
        .flatMap(([key, value]) => [value].flat().map((v) => [key, v]))
        .map((it) => it.join("="))
        .join("&");
    // Construct the full URL with the query string
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/ecommerce/products?${queryString}&entityTypeId=${entity.id}&brands=${brand.id}&limit=12`;
...
async function getLinked(entity, brand) {
    // Construct the full URL with the query string
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/ecommerce/client/linkedEntityTypeBrands?brandSlug=${brand}&entityTypeSlug=${entity}`;
...
const Sellerpage = async ({ params, searchParams }) => {
    ...
    const { result: entity } = await getEntity(categorySlug);
    const { result: brand } = await getBrand(brandSlug);
    const products = await getProducts(entity, brand, searchParams);
    ...
    const { result: linked } = await getLinked(categorySlug, brandSlug)
    return (
        <>
            <div className="container justify-start mx-auto mt-10 mb-10">
                ...
                <div className="text-3xl pb-0 px-4 flex justify-between items-center mb-4">
                    <h1 className="peyda text-[18px] font-bold">{entity?.name} {brand?.name}</h1>
                ...
                <div className="">
                    ...
                    {linked?.description && (
                        <div
                            ...
                            dangerouslySetInnerHTML={{ __html: linked?.description }}
                        ></div>
                    )}
                </div>
            </div>
