...
export default function Orders() {
  ...
  const getVendors = async () => {
    await fetcher({
      url: `/v1/api/ecommerce/user/vendors?sortOrder=DESC&offset=0&orderBy=id`,
      method: "GET",
    }).then((res) => {
      setVendors(res?.result);
      setReportProps((prevProps) => ({
        ...prevProps,
        vendorId: res?.result[0]?.id,
      }));
    });
  };
  ...
  const fetchTotal = async () => {
    ...
    let url = `/v1/api/ecommerce/report/vendorSales/total?beginDate=${reportProps.startDate}&endDate=${reportProps.endDate}`;
  ...
  let url = `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/ecommerce/report/vendorSales?beginDate=${reportProps.startDate}&endDate=${reportProps.endDate}`;
  if (reportProps.vendorId !== null) {
    url += `&vendorId=${reportProps.vendorId}`;
  }
  if (reportProps.variationPriceId !== null) {
