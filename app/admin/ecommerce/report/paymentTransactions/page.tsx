...
export default function Orders() {
  ...
  const {
    ...
    isLoading: paymentGatewaysIsLoading,
    error: paymentGatewaysError,
  } = useFetcher(`/v1/api/ecommerce/admin/paymentGateways`, "GET");
  ...
  const fetchTotal = async () => {
    ...
    let url = `/v1/api/ecommerce/report/paymentTransactions/total?beginDate=${reportProps.startDate}&endDate=${reportProps.endDate}`;

    if (reportProps.paymentGatewayId !== null) {
      url += `&paymentGatewayId=${reportProps.paymentGatewayId}`;
    }
  ...
  const columns = [
    ...
    {
      accessorKey: "payment.paymentGateway.name",
      header: "نام درگاه",
      size: 10,
      maxSize: 10,
    },
...

    {
      ...
      header: "مبلغ دریافتی از درگاه",
      size: 10,
      maxSize: 10,
      Cell: ({ row }) => Number(row.original.receivedAmount).toLocaleString(),
    },
    {
  ...
  let url = `${process.env.NEXT_PUBLIC_BASE_URL}/v1/api/ecommerce/report/paymentTransactions?beginDate=${reportProps.startDate}&endDate=${reportProps.endDate}`;
  if (reportProps.paymentGatewayId !== null) {
    url += `&paymentGatewayId=${reportProps.paymentGatewayId}`;
  }
  if (reportProps.orderId !== null) {
          ...
          <>
            ...
            <div className="flex-1">
              <FormControl fullWidth>
                <InputLabel id="demo-peyk-select">درگاه پرداخت</InputLabel>
                <Select
                  labelId="demo-peyk-select-label"
                  id="demo-peyk-select"
                  label="پیک"
                  value={reportProps.paymentGatewayId}
                  fullWidth
                  onChange={(e) => {
                    const newValue =
                    ...
                    setReportProps({
                      ...reportProps,
                      paymentGatewayId: newValue,
                    });
                  }}
                >
                  <MenuItem value={null}>بدون انتخاب</MenuItem>
                  {paymentGateways?.result?.map((value, index) => {
                    return (
              ...
              {total && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  ...
                  <div className="flex flex-row flex-wrap bg-gray-100 items-center px-3 py-4 justify-between text-center rounded-xl border border-primary">
                    ...
                    <span>
                      {Number(total?.totalDiscountFee).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex flex-row flex-wrap bg-gray-100 items-center px-3 py-4 justify-between text-center rounded-xl border border-primary">
                    <span>مقدار کل سفارش </span>{" "}
                    <span>{Number(total?.totalPrice).toLocaleString()}</span>
                  </div>
                  <div className="flex flex-row flex-wrap bg-gray-100 items-center px-3 py-4 justify-between text-center rounded-xl border border-primary">
                    <span>مقدار کمیسیون درگاه </span>{" "}
                    <span>
                      {Number(total?.paymentCommissionAmount).toLocaleString()}
                    </span>
                  </div>

                  <div className="flex flex-row flex-wrap bg-gray-100 items-center px-3 py-4 justify-between text-center rounded-xl border border-primary">
                    <span>مبلغ دریافتی از درگاه </span>{" "}
                    <span>
                      {Number(total?.receivedAmount).toLocaleString()}
                    </span>
                  </div>
                </div>
              )}
