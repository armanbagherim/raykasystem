...
export default function Discount() {
  ...
  useEffect(() => {
    setTitle({
      ...
      link: "/admin/ecommerce/factorDiscounts/new",
    });
  }, []);
    ...
    try {
      ...
      if (result.isConfirmed) {
        const req = await fetcher({
          url: `/v1/api/ecommerce/admin/factorDiscounts/${id}`,
          method: "DELETE",
        });
        toast.success("موفق");
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <>
          <a href={`/admin/ecommerce/factorDiscounts/${row.id}`}>
  ...
  return (
    <div>
      <LightDataGrid
        url={"/v1/api/ecommerce/admin/factorDiscounts"}
        columns={columns}
        triggered={triggered}
      />
    </div>
